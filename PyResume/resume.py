...
class Resume(FPDF):
    def __init__(self, config: DocumentConfig):
        super().__init__()
        self.config = config
        self.set_margins(self.config.get_margin('margin'),
    ...
    def divider_line(self):
        self.jump_y(1)
        self.line(self.config.get_margin('margin'),
                  self.get_y(),
                  self.config.get_margin('line_end'),
                  self.get_y())
        self.jump_y(1)
    ...
    def write_body(self, text: str):
        ...
        self.multi_cell(h=6,
                        txt=text,
                        **self.config.get_cell('multi'))

    def write_two_column_body(self, info: dict):
        self.set_font(**self.config.get_style('emphasis'))
        self.cell(h=6,
                  txt=info['name'],
                  **self.config.get_cell('left'))
        self.cell(h=6,
                  txt=info['years'],
                  **self.config.get_cell('right'))
        self.set_font(**self.config.get_style('body'))
        self.multi_cell(h=6,
                        txt=f'{info["accomplishment"]}',
                        **self.config.get_cell('multi'))
        self.jump_y(4)
